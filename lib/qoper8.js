...
var qoper8 = {
  ...
  useFork: function() {
    var fork = false;
    if (process.version.substr(1,4) !== '0.4.') fork = true;
    return fork;
  },
  ...
  startChildProcesses: function(callback) {
    ...
    var onChildProcMessage = function(callback) {
      if (qoper8.useFork()) {
        qoper8.process[pid].on('message', callback);
      }
      else {
        var contentStr = '';
        var dataStr;
    ...
    for (var i = 0; i < this.poolSize; i++) {
     ...
     // *****************************************************
      ...
      onChildProcMessage(function(response) {
        //if (qoper8.trace) console.log("child process returned response " + response);
        if (!qoper8.useFork()) response = JSON.parse(response);
        if (response.ok) {
      ...
      while (pid) {
        ...
        if (!pid) {
          ...
          if (qoper8.useFork()) {
            ...
            var message = JSON.stringify(queuedRequest.action);
  ...
  childProcess: {
    handler: function(actionMethod, isAsync) {
      ...
      var sendResponse = function(response) {
        var json = {ok: process.pid, response: response};
        if (qoper8.useFork()) {
          process.send(json);
        }
        else {
      ...
      onParentMessage(function(action) {
        if (!qoper8.useFork()) action = JSON.parse(action);
